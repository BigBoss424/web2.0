/*!
 * @nuxt/server-edge v2.11.1-26378723.c5d0067 (c) 2016-2020
...
async function renderAndGetWindow (
  ...
  await utilsEdge.timeout(new Promise((resolve) => {
...
const nuxtMiddleware = ({ options, nuxt, renderRoute, resources }) => async function nuxtMiddleware (req, res, next) {
  // Get context
  const context = utilsEdge.getContext(req, res);
...
class ServerTiming extends utilsEdge.Timer {
  constructor (...args) {
    super(...args);
    this.headers = [];
  }
...
class Server {
  constructor (nuxt) {
    ...
    this.publicPath = utilsEdge.isUrl(this.options.build.publicPath)
  ...
  async ready () {
    ...
    const { VueRenderer } = await new Promise(function (resolve) { resolve(_interopNamespace(require('@nuxt/vue-renderer-edge'))); });
