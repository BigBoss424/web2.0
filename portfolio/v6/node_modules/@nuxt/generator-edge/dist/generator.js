/*!
 * @nuxt/generator-edge v2.11.1-26378723.c5d0067 (c) 2016-2020
...
class Generator {
  constructor (nuxt, builder) {
    ...
    this.distNuxtPath = path.join(
      this.distPath,
      utilsEdge.isUrl(this.options.build.publicPath) ? '' : this.options.build.publicPath
    );
  }
  ...
  async initRoutes (...args) {
    ...
    if (this.options.router.mode !== 'hash') {
      try {
        generateRoutes = await utilsEdge.promisifyRoute(
    ...
    let routes =
      this.options.router.mode === 'hash'
        ? ['/']
        : utilsEdge.flatRoutes(this.options.router.routes);
  ...
  decorateWithPayloads (routes, generateRoutes) {
    ...
    generateRoutes.forEach((route) => {
      ...
      const path = utilsEdge.isString(route) ? route : route.route;
      routeMap[path] = {
        route: path,
